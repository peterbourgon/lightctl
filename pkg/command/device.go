...
func DeviceSetLight(gateway *url.URL, stdout, stderr io.Writer) *ffcli.Command {
	return &ffcli.Command{
		...
		Subcommands: []*ffcli.Command{
			...
			DeviceSetLightRed(gateway, stdout, stderr),
		},
		Exec: func(ctx context.Context, args []string) error { return flag.ErrHelp },
	}
}
...
func DeviceSetLightRed(gateway *url.URL, stdout, stderr io.Writer) *ffcli.Command {
	fs := flag.NewFlagSet("lightctl device set light red", flag.ExitOnError)
	var (
		id         = fs.Int("id", 0, "device ID")
	...
	return &ffcli.Command{
		...
		ShortHelp:  "Set light control mireds (white spectrum color) of a device",
		FlagSet:    fs,
		Exec: func(ctx context.Context, args []string) error {
			c, err := config.Load()
			if err != nil {
